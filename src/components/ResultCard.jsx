...
const ResultCard = (props) => {
  ...
  const addOption = (type) => {
    ...
    if (type === "objective") {
      ...
      return choices.map((obj, idx) =>
        <div className="objective-card"key={idx}>
          ...
          <Progress
            percent={Math.round((dict[obj] / sum) * 100)}
            status="active"
          />
        </div>
      );
    ...
    // 객관식 (사진)
      ...
      return Array.from({length: img_count}, () => "")
        .map((obj, idx) =>
          <div className="img-objective-card" key={idx}>
            ...
            <Progress
              percent={Math.round((dict[idx] / sum) * 100)}
              status="active"
            />
          </div>
        )
    ...
    // 주관식
      ...
      return (
        <div className="subjective-card">
          <Table className="subjective-table"
            columns={columns}
            dataSource={data}
            scroll={{y: 240}}
          />
  ...
  return (
    <div className="resultcard-wrapper">
      <Badge.Ribbon
        ...
        color={badge_color}
      >
        <Card title={<div className="resultcard-title">{`${num}. ${title}`}</div>}
              size="small"
        >
