    ...
    try {
      ...
      for (const item of res.data.questions) {
        if (item.type === "objective") {
          const parsed_choices = item.choices.map((choice: any) => ({text: choice}))
          ...
          for (let i = 0; i < item.img_count; i++) {
            // blob 데이터를 파일로 변환
            const image_file = new File([new Blob([new Uint8Array(db_images.data[i].data.data)])],
              db_images.data[i].original_name, {type: db_images.data[i].mimetype})
            images.push({
              file: image_file,
  ...
  const handleSubmit = async () => {
    ...
    if (JSON.stringify(survey2) === JSON.stringify(result)) {
      ...
      for (const item of img_objective) {
        ...
        await api.post(`/api/image/upload_img_obj?survey_id=${survey.id}&question_index=${item.question_index}`, formData);
      }
          ...
          <div className="survey-wrapper">
            ...
            >
              <div className="survey-header">
                <Menu mode="horizontal" className="surveyinfo-menu" selectedKeys={["2"]}>
                  <Menu.Item key="1" icon={<DesktopOutlined/>} className="menu-item"
                             onClick={() => {
                               props.history.push(`/mysurvey/${survey.id}`)
                             }}
                  >
                    등록된 화면
                  </Menu.Item>
              ...
              {values.questions.map((item: any, index: number) => {
                ...
                if (item.type === "objective") {
                  const choices = item.choices.map((choice: any, idx: number) =>
                    <div key={idx}>
                      <div className="obj-choice">
                        ...
                        <CloseOutlined
                          ...
                          onClick={() => {
                            const delete_choice: any = {...values};
                    ...
                    <div key={idx}>
                      <div className="img-obj-choice">
                        ...
                        <CloseOutlined
                          ...
                          onClick={() => {
                            const delete_img_choice: any = {...values};
                            delete_img_choice.questions[index].images.splice(idx, 1);
                            setValues(delete_img_choice);
                          }}
                        />
                      </div>
                      <div className="error-message" style={{textAlign: "center", fontWeight: "bold", color: "khaki"}}>
              ...
              <div className="survey-footer">
                ...
                <div className="submit-button">
                  <Button type="primary" size="large" danger style={{marginRight: "12px"}}
                          onClick={showModal}
                  >
                    삭제하기
                  </Button>
                  ...
                  <Modal title="설문조사 삭제하기" visible={isModalVisible}
                         onOk={handleOk} onCancel={handleCancel}
                  >
                    <p>정말 삭제하시겠습니까?</p>
                  </Modal>
                  <Modal title="설문조사 수정하기" visible={isModalVisible2}
